...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    vector<int> appleLocation(apples.size()), orangeLocation(oranges.size());
    for (int i = 0; i < apples.size(); i++){
        appleLocation[i] = a + apples[i];
    }
    for (int i = 0; i < oranges.size(); i++){
        orangeLocation[i] = b + oranges[i];
    }
    int appleCounter = 0, orangeCounter = 0;
    for (int i = 0; i < appleLocation.size(); i++){
        if (appleLocation[i] >= s && appleLocation[i] <= t){
            appleCounter++;
        }
    }
    for (int i = 0; i < orangeLocation.size(); i++){
